    ...
    ILogger<DeclarationService> logger)
{
        ...
        Guid id, Guid officerUserId, string visaNo, string? comment, CancellationToken ct)
    {
        ...
        var outcome = await Transition(
            ...
            d =>
            {
                ...
                DeferredDocuments.Queue(outbox, deferred, visaNo);
            },
            officerUserId,
            AuditActions.DeclarationApproved,
            detail: new { VisaNo = visaNo, Documents = deferred.Count, CommentSha256 = CommentHash(comment) },
            ct);
        ...
        Guid id, Guid officerUserId, string? comment, CancellationToken ct)
    {
        ...
        var outcome = await Transition(
            ...
            detail: new { CommentSha256 = CommentHash(comment) },
            ct);
    ...
    /// <summary>
    /// design.md §9's "approval/rejection with comments **hash**" (slice 7.2). Until this slice the
    /// detail carried a bare <c>HasComment</c> boolean, which answers no question a claims dispute
    /// asks: it cannot show that the comment on screen today is the comment the officer wrote, which
    /// is the whole point of hashing rather than storing it twice.
    /// </summary>
    /// <remarks>
    /// The trimming is not incidental — it must hash **exactly** the text <see cref="AddComment"/>
    /// stores, or the two drift on the first comment somebody types with a trailing newline and the
    /// hash proves nothing about the row beside it. Null when the comment is blank, for the same
    /// reason <see cref="AddComment"/> writes no row: there is nothing to attest to. Unsalted SHA-256
    /// via <see cref="TokenHashing"/>, the same function §9's credentials use, because this is a
    /// checksum over text somebody can already read and not a secret.
    /// </remarks>
    private static string? CommentHash(string? comment) =>
        string.IsNullOrWhiteSpace(comment) ? null : TokenHashing.Hash(comment.Trim());
