...
public static class MediaValidation
{
        ...
        out string clarityResult)
    {
        ...
        if (declaredContentType is null
            || !media.AllowedFor(rule.Kind).Contains(declaredContentType, StringComparer.OrdinalIgnoreCase))
        {
        ...
        if (ImageHeader.LooksLikePdf(headerPrefix))
        {
            ...
            return string.Equals(declaredContentType, ImageHeader.Pdf, StringComparison.OrdinalIgnoreCase)
                ? MediaRejection.None
                : MediaRejection.ContentTypeMismatch;
        }
        ...
        if (status != ImageHeaderStatus.Ok
            || !string.Equals(info.ContentType, declaredContentType, StringComparison.OrdinalIgnoreCase))
        {
