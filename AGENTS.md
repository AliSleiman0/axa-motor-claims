# AGENTS.md — AXA Motor Claim Management
...
- **Android release build: `scripts/android-release.ps1`, never `gradlew assembleRelease` by hand.** It asserts the four things that each produce an APK which installs, runs and is wrong: `CAP_SERVER_URL` unset (set, `capacitor.config.ts` adds `server.url` + cleartext + WebView debugging), a real non-placeholder `android/app/google-services.json`, a `keystore.properties` (absent, Gradle emits an *unsigned* APK rather than falling back to the debug key), and a JDK 21. **`JAVA_HOME` must point at JDK 21**: the Capacitor plugins pin that toolchain, this machine's PATH `java` is 17, and Gradle does *not* auto-detect `C:\Program Files\Microsoft\jdk-21.*` — the failure is `Cannot find a Java installation ... {languageVersion=21}` minutes into a build.
- **Never commit:** the keystore, `keystore.properties`, the real `google-services.json` (all three now actually gitignored — the stock Capacitor template left them commented out), or the FCM service-account JSON. The last is a credential on the *server* side, mounted like the VAPID private key; `google-services.json` is not secret but is environment-specific. `android/app/google-services.json.placeholder` is the committed template, and Gradle skips the plugin on any file still carrying the `PLACEHOLDER` marker so a copied template is a warning, not a broken build.
- **`appId` is `com.axa.motorclaims`** — a client literal outside `appsettings.Placeholders.json`, and a deliberate exception: an applicationId is compiled into the APK, is what the Firebase app is registered against, and is what every installed handset knows, so it cannot be swapped at deployment. Changing it means a new Firebase app and a reinstall on every phone.
- **The FCM credential is provable without a handset.** `FcmAccessTokensLiveTests` mints a real access token from Google and skips unless two env vars are set — the `AzuriteFact`/`SandboxFact` shape, third outing. It is the only thing that exercises the PEM import, the RS256 signature and the claim set against the real service; everything else is scripted, and the disposed-`RSA` bug this adapter shipped with passed every stubbed test and would have failed ~an hour after each deploy. Run it with:
  ```
  $env:FCM_LIVE_PROJECT_ID = "axa-motor-claims-dev"
  $env:FCM_LIVE_SERVICE_ACCOUNT_JSON = "C:\dev\secrets\axa-fcm-service-account.json"
  ```
- Handset-side reasons Android push stops (OEM battery killing, and Huawei having no Play Services at all): **`docs/oem-push-guidance.md`**, including the support triage order.
