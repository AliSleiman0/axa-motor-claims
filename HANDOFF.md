...
> ## ⏭️ NEXT SESSION: commit slice 2.5, then start week 3 (3.1 voice note + damage diagram)
> Build proceeds per `docs/build-playbook.md` — week 1 is 1.1–1.5 ☑, **week 2 is 2.1–2.5 ☑** (2.1–2.4 committed; 2.5 pending commit); next unticked slice is **3.1**. Slice learnings are in the playbook's Notes lines. **Week 3 starts with the fidelity rule** (playbook §"How to use it"): run one short session upgrading the week-3 cards to verbatim prompts, using the Notes from weeks 1–2, *before* building 3.1.
>
> > **One trap disarmed for 5.1:** `useCapture` reports `configLoaded` and `ready` as **two** flags. `findBucket` returns undefined both while the config is in flight and when a bucket is absent from §7.1's registry — collapsed, the second case renders "Loading photo settings…" for ever, with a clean network tab and an empty console. The garage buckets 5.1 adds need a `MediaBuckets` entry **and a migration** (`bucket` is check-constrained), so wiring the screen first is the likely order; the panel now says the section is unconfigured instead.
>
> **3.1 inherits from 2.5:** the whole media pipeline is now client-to-`sent`. The voice note and the damage diagram are two more artifacts through the **same** §7 path — `POST /api/expert/assignments/{id}/documents` with `bucket`, `origin`, file **in that order** — but neither is a photograph, so neither goes through the clarity gate: §7.2 item 4 makes voice a playback-confirm, and a canvas-exported PNG has no focus to check. Both need a **new bucket, which is a migration** (`bucket` is a check-constrained enum, and `MediaBuckets` is §7.1 as data), plus their `Next3:DocTypes` placeholder keys, which already exist (`VoiceNote`, `DamageDiagram`). §5.1 says both land in *Expert documents*. Audio acceptance is **#10 and unanswered** — the fake accepts it; do not invent a format list, and note `Media.ImageContentTypes`/`DocumentContentTypes` have no audio entry today. The diagram ships **functional, not polished** (§1 — its polish was spent to fund Option 2, so resist making it nice). `CapturePanel`/`useCapture` are reusable but assume an image; a voice recorder is a sibling, not a variant — do not widen the hook and cost 5.3 its reuse.
>
