...
> ## ⏭️ NEXT SESSION: commit slices 2.2 + 2.3, then slice 2.4 (expert screens E1/E2 + Arrived)
> Build proceeds per `docs/build-playbook.md` — week 1 is 1.1–1.5 ☑, week 2 is 2.1–2.3 ☑ (2.2 and 2.3 pending commit); next unticked slice is **2.4**. Slice learnings are in the playbook's Notes lines.
>
...
src/
  ...
  Api.Tests/                xUnit: NetArchTest boundary rules (with planted-violation self-tests) +
                            integration tests on LocalDB via WebApplicationFactory (263 tests).
