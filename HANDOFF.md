...
> ## ⏭️ NEXT SESSION: review and commit slice 3.4 — then **week 4**, which is the client demo
> Build proceeds per `docs/build-playbook.md` — weeks 1 and 2 are ☑ and committed, and so are 3.1, 3.2 and 3.3 (`fa5c707`); **3.4 is ☑ and pending commit, and that completes week 3**. Next unticked slice is **4.1** (the declaration state machine), and **week 4's cards are still outline form** — run the fidelity-rule session first: *"Upgrade the week-4 cards in build-playbook.md to verbatim prompts, using the Notes from completed slices."*
>
...
3. **Send the NEXT3 OpenAPI proposal** (§6). Fastest way to force clarity on the biggest unknown. **✅ WRITTEN — `docs/next3-openapi.yaml` (slice 3.3, 2026-08-21). It is finished and unsent; send that file.** It specifies the nine calls plus the inbound assignment callback, makes `clientRef` a *required* parameter with the dedupe stated as a requirement on NEXT3 (#32), asks for a sandbox server (#1), and surfaces #2/#5/#6/#8/#12/#34/#42 in the description text so an AXA reviewer meets them. It validates clean under `redocly lint`. Note it deliberately matches the **client document v0.5 §3.1 endpoint list**, not design.md §6.1's original seven — §6.1 was corrected to match, so the two documents AXA receives agree.
