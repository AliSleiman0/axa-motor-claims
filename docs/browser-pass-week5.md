...
| 9 | **`/p/{token}` renders a completely blank page.** B3 shows the broker `http://localhost:5173/p/{token}` to hand to a customer; there is no `/p/:token` route **and no catch-all** in `App.tsx`, so any unknown path paints bare canvas. | 5.3 builds the route — that part is expected. The gap is that there is no not-found route at all, and B3 is the first screen that *hands out* a URL. A customer opening it today sees nothing, not an explanation. | 5.3 for the route; a catch-all is its own small ticket. |
