...
**2026-08-27, slice 7.2** — the last four, and the file is now closed. **7**: the phone number is
hidden below 480 px as well as on the touch shells, so the officer, admin, broker and desktop-expert
headers keep **Sign out** at 390 px; the two rules are complementary and both are kept. **8**: the
push panel no longer trusts `getSubscription()` — it re-posts whatever the browser holds and claims
"Notifications are on" only once the server has accepted it, which fixes the database-reset case the
finding widened the ticket to cover. **9**'s remaining half: `pages/NotFoundPage.tsx` with
`path="*"` last in `Routes`, so a mistyped `/p/{token}` renders an explanation instead of bare
canvas. **10**: `CancelledRequestMiddleware`, outermost, swallowing a fault only when the client has
already gone — the guard is `RequestAborted`, never the exception type, because an aborted
`SqlCommand` surfaces as `SqlException` or `TaskCanceledException` interchangeably.
