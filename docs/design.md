...
**The three §5.2 rows above all carry `PushTiming.OnApproval`** (a field on `BucketRule`, realized slice 4.1): they are stored `push_status = deferred` with no outbox row, and §5.2's approve transition enqueues them once a visa exists. §7.1's own garage rows predate that distinction and are kept as the BRD's statement of provenance; the repair buckets and the discharge/invoice split are slice 5.1's. The `approval_image` row is #18's "approval and comments captured as an image" — `AllowUpload: false` not because §7.1 marks it capture-only but because the officer's browser renders it and there is no file to pick (3.1's distinction), and an *image* so §7.2's server floor applies to the render exactly as to a photograph.
