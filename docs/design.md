...
|---|---|---|
| `app_user` | Authoritative | One row per authenticated identity. `id`, `phone` (unique, E.164), `role` (expert/garage/claim_officer/broker/admin), `display_name`, `status` (invited/active/inactive/**sync_blocked**, added slice 7.5 — §12 #7/#8), `inactivated_at`, `created_at`. **`status` is the EF concurrency token since slice 7.5** — two independent writers now flip it (`AdminUserEndpoints.Deactivate`, `MasterDataSyncTask`) and a stale read-then-write on either side is a lost update with no arbiter otherwise (db-review finding, same reasoning as `declaration.state`/`document.push_status`) |
| `expert_profile` | **Hybrid** — seeded from NEXT3 `GET /experts`, app-authoritative for app fields | `user_id` FK, `next3_id` (nullable until mapped; filtered unique index — same on `garage_profile`; realized 2026-08-19, slice 1.3), `email`, `active`, `inactivated_at`. **Sync job built slice 7.5** (`Api.Modules.Users.MasterDataSyncTask`, #7/#8): NEXT3 wins on identity fields (name, mobile, email) for a *new* profile it creates; the app's `active`/`app_user.status` are governed by the sync task once mapped, except a status of `inactive` (admin-deactivated), which the sync task never touches — see §12 #7/#8 |
...
- Volumes for sizing: **claim volume resolved 2026-08-31 (#20)** — real measurement over 200 days gives **average 15 claims/day, maximum 32/day**, well below the earlier ~100/day placeholder. Photo count/size assumption (~15 × 1.5 MB) and user count (~200) remain unconfirmed; the client's 2026-08-31 answer expects user count to run higher once experts, garages and brokers are all onboarded, with no firm number given.
...
**Known gap, carried:** the gate reads the declaration and the document row commits later in the same request, so an upload can be admitted before a decision and land after it — a `deferred` row nothing will ever enqueue. The upload now carries `state` as a concurrency token, which closes the wide half of that window (the seconds a file spends streaming), and approve's read of the deferred set moved to *after* its NEXT3 verification, which removes the other wide half (a call with a 30-second timeout). What remains is the microseconds between that read and its commit. The permanent fix is a **re-queue sweep** — any `deferred` document on a declaration that already has a visa is by definition stranded — and it is **built in slice 7.2** as `StrandedDeferredRequeueTask` (§7.3's fifth sweep). The window is not closed; it is *drained*, hourly, which is the honest description: the microseconds still exist, and nothing that falls into them stays there. **`document.push_status` became the EF concurrency token in the same slice** and that is what makes the sweep safe to run twice at once — `CleanupWorker` lives in the API host and §10 pins `minReplicas: 1`, not `maxReplicas: 1`, so two replicas really do select the same rows; the unique index on `outbox_message_id` catches nothing there, because each writer generates a *different* Guid and it is a lost update rather than a collision. 4.1's objection to a token on `arrived_at` does not apply: a token punishes every write of the entity, and the only other writers of a `document` row are approve's flip (the same column, wanting the same guard) and `DocumentBlobSweeper`, whose sets are disjoint from the deferred one.
...
- **A2 failed-push queue:** lists `next3_outbox` where `status='failed'` (and long-`pending`), showing operation, claim/visa, attempts, `last_error`, timestamps; **Retry** resets to `pending` with `next_retry_at = now`. ~4 hours of work; it is the safety net, the debugging tool, the support answer, and the feature AXA will value more than half the BRD. **Realized 2026-08-25, slice 6.2**, in `Api.Outbox` rather than an admin module — architecture rule 4 lets no other namespace reference `Next3OutboxMessage`, and the rule is right, so the surface moved to the row instead of the row leaking out; what leaves the namespace is a projection.
  ...
  **The overdue arm, realized 2026-08-27, slice 7.2** — 6.2's recorded blind spot, closed exactly as it was described: a third predicate arm plus a grace period. `Listed` becomes `failed ∨ (pending ∧ (next_retry_at > now + PollSeconds ∨ next_retry_at < now − 2 × PollSeconds))`. A row due in the *past* and not being claimed is the signature of a stopped worker job, or of a backlog draining slower than it fills — the case where "where did that photograph go?" is most urgent and A2 was answering "nothing has failed". **Two polls of grace, and it is load-bearing rather than round:** every row is enqueued with `next_retry_at = now`, so at one poll a healthy queue would flash rows overdue in the seconds before the next tick, and an alarm that fires during normal operation is worse than no alarm. It cannot collide with the long-pending arm either — a claimed row's `next_retry_at` is pushed a full `LeaseSeconds` out and it is `processing`, which A2 never lists.

  **`/count` returns `{ failed, overdue }` and the badge sums them; long-`pending` rows are still counted by neither.** That is 6.2's argument kept rather than reversed: a long-pending row clears itself, so a number that rose and fell with the backoff schedule would be an alarm nobody trusts — while an overdue row clears itself only if somebody restarts something, which is precisely what a badge is for.

  **The list stops at `ListLimits.MaxRows` (200) and Retry all deliberately does not** (slice 7.2). The divergence is the direction that matters: a `Take` inside the shared predicate would make the *button* partial, so an admin would press Retry all during an outage, see a 200, and be silently left with a backlog it never touched. The screen truncates; the button covers everything the predicate covers; a test asserts both halves rather than a comment claiming them. 6.2's "exactly the list predicate" sentence is superseded to this extent and no further.
...
- **What that deliberately does not cover:** an abandoned or expired Option 2 request — and **slice 5.3 turned that from hypothetical into real, so the sentence changed with it.** 5.2 could say the case "carries no documents at all today", because `public_document` did not exist; it does now, and a customer who photographs their identity card and then never presses Send leaves those bytes in the transit container with nothing to move them on. **Slice 6.1 widened that from one file to as many as six** — the identity card and five photographs of an identifiable car, plate included — without changing the mechanism or the answer needed. Sweep 1 needs an outbox row a `PushTiming.Never` bucket never has; sweep 2 spares any blob a live `document` row claims; sweep 3 needs `emailed_at`, which only B4's Send writes. The delete itself is not the hard part — the sweep keys on the owner kind, so those rows join its set the moment `emailed_at` lands — what is missing is a rule for the requests where it never will. How long a member of the public's identity documents are kept after they never finished is a **client answer (#4/#22)**, the same shape and now the same weight as the rejected-declaration gap above. **Both are 7.2 tickets**, and both are written down rather than left as an absence somebody has to notice. **Both are built in slice 7.2**; the second as a second branch of `BrokerMediaCleanupTask`, keyed on the request's **newest `public_link_token.expires_at` + `Retention.AbandonedRequestBlobDays`** rather than on a state, because `expired` is computed in B1's projection and never written.

  **That branch is restricted to `link_issued` and `customer_in_progress`, and the restriction is the safety property** (raised by the db-review). Keyed on `emailed_at IS NULL` alone it also matched a **`ready_to_send`** request — a *completed* submission sitting in B4 waiting for a human to press Send. Nothing expires one, so its token lapses on schedule and the sweep would have deleted the attachments while the button was still on screen: `BrokerRequestEmail.Attachments` then refuses `409 attachments_unavailable` for ever, the customer's link is locked, and no second copy of anything exists. Naming the two pre-submission states instead means a resendable failed send (`sent` with a null `emailed_at`, §5.3's ordering) is protected **outright rather than for a window** — stronger than the ticket asked for, and the direction a retention rule should err in. **What it leaves open, deliberately:** a submission the broker never reviews keeps its bytes indefinitely, because "how long does a broker have?" is the same #4/#22 question and not one to invent inside a sweep.
...
**Audit trail.** The BRD omits one; claims disputes make it mandatory. `audit_log` (append-only) records: login success/failure, every media upload (who, which claim/declaration/request, when, origin flag), Arrived presses with coordinates, every declaration transition with actor, approval/rejection with comments hash, broker email sends with recipient, admin CRUD, outbox retries from A2, public-page submissions (actor null, token id logged).

**Seven of those were prose until slice 7.2, and one was a boolean.** The comments hash is now `CommentSha256 = TokenHashing.Hash(comment.Trim())` — exactly the text `AddComment` stores, null when blank — replacing a `HasComment` flag that answers no question a dispute asks. And the §8 device registry, which had no trail at all, gains `device_token_registered` / `device_token_revoked` (detail `Reason: user | evicted | deactivated`) / **`device_token_displaced`** (actor = the *new* registrant, entity = the displaced row, detail `PreviousUserId` — the handset-changing-owner event) / `push_subscription_registered` / `push_subscription_removed`, plus `refresh_token_family_revoked` (§4's reuse branch — the app's only automatic "somebody may be replaying a stolen credential" signal, and it was silent) and `broker_email_send_failed` (§5.3's compensating write, which released the claim on `emailed_at` and only logged). Entity kinds `device_token` and `push_subscription`.

**Registration audits fire on creation, displacement and revocation only, never on the refresh path** — the Capacitor shell re-registers on every launch, and auditing that would bury the events worth reading under one row per app open per handset per day. **Document *reads* stay unaudited** (scope-decisions, 7.2): §9 enumerates uploads, and read-auditing multiplies writes by document count per screen view to answer no dispute question.

**Two ordering rules, because both are traps.** `AuditWriter` joins the caller's transaction, so a row staged before a save that lands in a `DbUpdateException` catch is discarded by `ChangeTracker.Clear()` while the endpoint still answers 200 — the appends therefore go *after* the successful save, or after the clear where the write itself is compensating. And where the write is an `ExecuteUpdate`/`ExecuteDelete` there is no transaction to join, so deactivation's device revocations became **tracked** writes in order to commit with their audit rows rather than beside them. This is the InfoSec answer to "who uploaded which photo, when".
...
**Response headers (realized 2026-08-27, slice 7.1).** Every response carries `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Content-Security-Policy: default-src 'none'; frame-ancestors 'none'`, `Referrer-Policy: no-referrer` and `Strict-Transport-Security: max-age=31536000`, from `SecurityHeadersMiddleware` (`src/Api/Infrastructure/`). Until 7.1 the API sent **none of them**. Three details are load-bearing rather than conventional. It is registered **first among everything that writes a response** — since slice 7.2 one middleware sits outside it, `CancelledRequestMiddleware`, which writes nothing a live client will ever read — because the two responses most worth covering never reach a handler — the rate limiter writes its own 429 and `PublicBodySizeMiddleware` its own 413, both short-circuiting everything downstream, and both are what an abusive caller sees most of. It writes from `Response.OnStarting`, so the assignment lands *after* every handler has written its own headers and `DocumentContent`'s own `nosniff` line stays a single value rather than becoming a duplicate. And there is **no configuration knob**: each value is right everywhere or wrong everywhere, and `Strict-Transport-Security` in particular is unconditional because a browser ignores an HSTS header received over plain `http`, so a switch would only be a way to deploy the wrong setting. `Referrer-Policy` is the one aimed squarely at §9.1 — the public page carries a live 256-bit credential in its URL, and a referrer header would hand that token to any third-party host the page ever touched. **§10/7.3 carve HTML out of the CSP** when the SPA moves in behind the same origin; the strict policy is correct for what the API serves today, which is JSON plus document bytes the browser fetches through the authorized client and renders from a `blob:` URL (§5.2).

**Cancelled requests (realized 2026-08-27, slice 7.2).** `CancelledRequestMiddleware` (`src/Api/Infrastructure/`) is the outermost middleware: `catch (Exception) when (context.RequestAborted.IsCancellationRequested)` swallows the fault and stamps **499** if nothing has been sent. The week-5 browser pass logged three of these in one session — react-query passes an abort signal through to `fetch` and React StrictMode double-mounts in development, so the first mount's query is cancelled mid-flight and surfaces as an unhandled `SqlException` and a 503. A user navigating away from a slow screen does the same thing in production. The harm is not the status nobody receives; it is that a real fault becomes indistinguishable from somebody changing their mind, in the one place anybody looks when something has gone wrong. **The guard is `RequestAborted`, never the exception's type** — an aborted `SqlCommand` surfaces as `SqlException` or `TaskCanceledException` interchangeably. **It introduces no global handler**: with a live client the exception propagates exactly as before, which §9.1's uniform surface relies on, and the consequence recorded in 7.1 stands — an app-faulted 500 still carries no security headers, because Kestrel skips `OnStarting` when the application threw.

**A global request-body ceiling (realized 2026-08-27, slice 7.2).** `ConfigureKestrel` sets `MaxRequestBodySize` to `(Media:MaxFileMb + 1) MB`; before it, Kestrel's 30 MB default applied to every authenticated upload, so a caller could stream 29 MB through TLS and the whole pipeline before the multipart reader refused it. One megabyte of headroom covers the multipart envelope, so a legal 15 MB file is never refused by the wrong limit with the wrong error. **Read once at startup and deliberately not reloadable** — a Kestrel limit is fixed when the server is built, so an `IOptionsMonitor` here would be a promise the server cannot keep. `/public/*` still lowers it per request through `IHttpMaxRequestBodySizeFeature`, which can only lower and never raise.

**List bounds (realized 2026-08-27, slice 7.2).** `ListLimits.MaxRows = 200` — a `const`, not a config key, because how many rows a screen may return is not a value AXA owns — caps E1, G1, O1, B1, the four A1 profile lists and A2's list. Every one of them projected an entire table before. No paging UI and no truncation flag this week (scope-decisions), resting on 6.2's argument that a page showing 200 of 3,000 rows above a **Retry all** that retried all 3,000 is more dangerous than the truncation. The A1 lists also gained an `ORDER BY` — they had none — alphabetically by display name, applied in memory after the cap so the comparison is a stated one rather than whatever collation the database carries.

**CORS is deliberately absent.** There is no `AddCors`/`UseCors` anywhere and none is added: the app is same-origin (§10 serves the SPA from the API's own host), so no browser ever needs a cross-origin grant, and adding one would be a decision that some other origin may call an API which includes the public surface. Recorded as a decision rather than left as an omission, and pinned by a test — a permissive default arriving later goes red.
...
**Token.** 256-bit random from a CSPRNG, base64url in the URL (`/p/{token}`); only the SHA-256 hash stored (`public_link_token.token_hash`) — a DB leak does not leak live links. **Reusable until first successful submission, then locked** (`locked_at`): the customer may leave and return to an unfinished form, but each link accepts exactly one submission. **Validity `PublicLink.ValidityDays` = 7 (placeholder)**. Invalid, expired, and locked tokens all return the **same uniform 404** — no distinguishable states to enumerate. Brokers can reissue (which creates a new request + token); reissue does not resurrect a locked one. **Uniform in the response headers too, since slice 7.1**: the pin behind that sentence compared only the *body* and the content headers, so a header that varied by failure mode — the security headers above, or anything a later slice adds beside them — would have been a distinguishable state the test whose whole job is to see differences could not see. `Date` and `Server` are excluded from the comparison as a clock and a host rather than a fact about the token. The **HTTP-level expiry boundary** is pinned in the same slice: the rule that the instant of expiry is expired was proven as a pure function from 1.5, and never that the endpoint reads the same clock the token was issued against.
...
| 5 | ~~API vs shared-directory + DB insert~~ | **Resolved 2026-08-31: API only** ("API call", client's clarification answer Q2 — see `docs/client-answers-2026-08-31/`). API-shaped interface stands as designed; no directory-writer/DB-insert path in `RealNext3Client` will be built | Nothing — the design already assumed this path. `#36` (NEXT3's DB engine) is now moot, since it existed only for the directory+DB case |
| 6 | Arrived field names/formats — **and which clock the date and time are in** | **Field mapping resolved 2026-08-31** (client answer Q6): table `CARS_LOSS_TOWING`, `DISTRIBUTION_LOSS_ARRIVED` (Y/N) + `DISTRIBUTION_LOSS_ARRIVED_DATE` (Date), keyed on `NOTIFICATION_ID` = the app's assignment id. **No GPS/location field was named** — recorded as a design decision, not a further question: arrival coordinates stay app-only (`expert_assignment.arrival_lat/lng`) and are omitted from `RecordArrival`'s NEXT3 payload. `Next3.Arrival*` placeholder mapping still carries `ArrivalTimeZone` — the zone the two fields above are read in is still unconfirmed. The outbox payload deliberately carries the **instant**, not a date-and-time pair: an expert arriving at 01:30 GST would otherwise be queued as arriving the previous day, and a queued row cannot be repaired from a value already collapsed into the wrong zone (realized 2026-08-20, slice 2.4) | Field mapping is now known; **the zone split** in the real client still needs `ArrivalTimeZone` confirmed |
...
| 8 | ~~Master-data extract mechanism (experts + garages; claim officers/brokers explicitly not sourced from NEXT3)~~ | **Resolved 2026-08-31, built slice 7.5**: `INext3Client.GetExperts`/`GetGarages`, `Api.Modules.Users.MasterDataSyncTask` reconciles `expert_profile`/`garage_profile` daily (`Next3:MasterDataSyncIntervalHours`, default 24). A new-in-network supplier gets a profile + SMS invite; one that drops out (or that NEXT3 marks inactive) sets `app_user.status = sync_blocked`, distinct from admin-driven `inactive` so a reappearance can never silently undo an admin's deactivation | Nothing further — `Next3:Mode` stays `fake`, so this runs against the fake's seeded experts/garages until real NEXT3 credentials land; no code change needed when they do |
...
| 41 | ~~One environment or two~~ | **Resolved 2026-08-31: two, confirmed** (client answer Q16). Designed for two (§10); costs assume it | Nothing — already built this way |
...
| 20 | ~~Volumes~~ | **Resolved 2026-08-31 for claims/day** (client answer Q15, measured over 200 real days): **average 15 claims/day, maximum 32 in a single day** — materially lower than the "~100 claims/day" placeholder assumption this document and Appendix A carried since week 0. Photo size: client recommends a standardized/capped size rather than device defaults (already true via `Media.MaxFileMb`). User count: expected **higher** than the 200-user assumption once experts, garages and brokers are fully onboarded — no firm number given | Sizing text below and in §1/§10 updated to the real figure; Azure hosting estimate can be revised down on claim volume (unchanged on user count pending a real number). Option 2 customer-link volume is still unasked |
| 43 | ~~Photo rules + fraud/tamper detection~~ | **Resolved 2026-08-31** (client answer Q9): no NEXT3-side limit on photo count, file size, format or resolution. **Fraud/tamper detection confirmed out of scope** — "to include the feature as a standalone feature with pricing and based on impact AXA to decide." Recorded in `docs/scope-decisions.md` | Nothing changes in the current build; a fraud-detection feature is priced separately only if AXA asks for it later |
...
```jsonc
{
  "Next3": {
    ...
    "MasterDataSyncIntervalHours": 24           // the client's own answer: "once per day" (#7/#8)
  },
  ...
  "Retention": {                             // (#4); the last four realized 2026-08-20, slice 2.3
    ...
    // Slice 7.2's three sweeps. All TBC (#4/#22) — how long AXA keeps a rejected claim's
    // photographs, or a member of the public's identity documents on a form nobody ever sent.
    "RejectedDeclarationBlobDays": 30,       // from declaration.decided_at            (§7.3)
    "AbandonedRequestBlobDays": 30,          // from the newest link token's expires_at (§7.3)
    "RevokedDeviceDays": 30,                 // prune of revoked device rows            (§8)
    "OrphanBlobHours": 24,
