...
4. **7.2's buffer is filling:** per-profile push subscription bug (**widened by the week-5 pass — a database reset triggers it for a *single* user, not just two users in one profile**), rejected-declaration blob retention, `pushsubscriptionchange`, E1 unbounded, narrow-iPhone layout check, A2 long-pending rows decision, **the 390 px overflow on office shells (4.4 fixed only `.app-shell--touch`, and 5.2 added a fourth office shell)**, **the stranded-`deferred` re-queue sweep and `CK_declaration_decision`'s blank-visa gap (5.1)**, **a catch-all route**, and **cancelled queries surfacing as unhandled `SqlException` → 503**. Keep the list in view when 7.2's card gets expanded.
